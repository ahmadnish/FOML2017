...
\paragraph{Quadratic Discriminant Analysis  (AN).} Qudratic Discriminant Analysis (QDA) follows the same step as in LDA, in which it results from assuming that all observation drawn from a Gaussian distribution, and using the Bayes' theorem to estimate the parameters to perform prediction. However, in contrast to LDA in which assumes a constant covariance matrix to each class, QDA assumes that each class has a different covariant matrix. This would result in a more complex model, thus more parameters to be calculated. Plus LDA is much less flexible than QDA which would result in less variance. However, the trade-off is when the K classes are actually does not share a common covariance matrix, which then would result in a high bias. Therefore, LDA would be better choice than if the training set is small and reducing variance is crucial. 
